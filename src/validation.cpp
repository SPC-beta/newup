...
{
    LogPrintf("AcceptToMemoryPoolWorker(), tx.IsSpend()=%s", ptx->IsSigmaSpend() || ptx->IsLelantusJoinSplit());
...
CAmount GetBlockSubsidy(int nHeight)
{
    ...
    if (nHeight <= params.utxo)
    {
    return 0 * COIN;// utxo blocks
    }
    else if (nHeight <= 4032) //until 1 week
    {
    return 4 * COIN;
    }
    else if (nHeight <= 8064) //until 2 weeks
    {
    return 8 * COIN;
    }
    else if (nHeight <= 12096) //until 3 weeks
    {
    return 12 * COIN;
    }
    else if (nHeight <= 17520) //until 1 month
    {
    return 16 * COIN;
    }
    else if (nHeight <= 35040) //until 2 months
    {
    return 14 * COIN;
    }
    else if (nHeight <= 52560) //until 3 months
    {
    if (nHeight == 43210)
    return (64000000 + 12) * COIN;
    else
    return 12 * COIN;
    }
    else if (nHeight <= 70080) //until 4 months
    {
    return 10 * COIN;
    }
    else if (nHeight <= 210240) //until 1 year
    {
    return 8 * COIN;
    }
    else if (nHeight <= 420480) //until 2 years
    {
    return 6 * COIN;
    }
    else if (nHeight <= 630720) //until 3 years
    {
    return 4 * COIN;
    }
    else
...
About 3Million BZX blocked from CREX24, new circulation total around 28.4Million BZX
...
until 1 month,  8/8 coins, adds 5424 blocks,   around 86784   coins, total created 182756  coins
until 2 months, 7/7 coins, adds 17520 blocks,  around 245280  coins, total created 428036  coins
until 3 months, 6/6 coins, adds 17520 blocks,  around 210240  coins, total created 638276  coins
until 4 months, 5/5 coins, adds 17520 blocks,  around 175200  coins, total created 813476  coins
until 1 year,   4/4 coins, adds 140160 blocks, around 1121280 coins, total created 1934736 coins
until 2 years,  3/3 coins, adds 140160 blocks, around 840960  coins, total created 2775716 coins
until 3 years,  2/2 coins, adds 140160 blocks, around 560640  coins, total created 3336356 coins
every 1 year,   1/1 coins, adds 140160 blocks, around 280320  coins added yearly
*/
...
CAmount GetMasternodePayment(int nHeight)
{
    ...
    if (nHeight <= params.utxo + 1)
    {
    return 0 * COIN;// utxo blocks  UTXO insertion, estimate 30M coins
    }
    else if (nHeight <= 4033) //until 1 week, around 7666 coins
    {
    return 2 * COIN;
    }
    else if (nHeight <= 8065) //until 2 weeks, adds 4032 blocks, around 16128 coins, total 23794
    {
    return 4 * COIN;
    }
    else if (nHeight <= 12097) //until 3 weeks, adds 4032 blocks, around 24192 coins, total 47986
    {
    return 6 * COIN;
    }
    else if (nHeight <= 17521) //until 1 month, adds 5424 blocks, around 43392 coins, total 91378
    {
    return 8 * COIN;
    }
    else if (nHeight <= 35041) //until 2 months, adds 17520 blocks, around 122640 coins, total 214018
    {
    return 7 * COIN;
    }
    else if (nHeight <= 52561) //until 3 months, adds 17520 blocks, around 105120 coins, total 319138
    {
    return 6 * COIN;
    }
    else if (nHeight <= 70081) //until 4 months,adds 17520 blocks, around 87600 coins, total 406738
    {
    return 5 * COIN;
    }
    else if (nHeight <= 210241) //until 1 year, adds 140160 blocks, around 560640 coins, total 967368
    {
    return 4 * COIN;
    }
    else if (nHeight <= 420481) //until 2 years, adds 140160 blocks, around 420480 coins, total 1387858
    {
    return 3 * COIN;
    }
    else if (nHeight <= 630721) //until 3 years, adds 140160 blocks, around 280320 coins, total 1668178
    {
    return 2 * COIN;
    }
    else
    {
    return 1 * COIN;            // after, every year, adds 140160 blocks, around 140160 coins
    }
}
        ...
        BOOST_FOREACH(const CTxIn &txin, tx.vin) {
            if(!txin.scriptSig.IsSigmaSpend()) {
                return state.DoS(
                    100, false,
                    REJECT_MALFORMED,
                    "CheckSpendBZXTransaction: can't mix privcoin spend input with regular ones");
            }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (pindex->nHeight >= chainparams.GetConsensus().CSV)
    {
    ...
    if (block.vtx[0]->GetValueOut() > blockReward && chainHeight > Params().GetConsensus().utxo)
    ...
    std::string strError = "";
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError) && chainHeight > Params().GetConsensus().utxo) {
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockSubsidy))
    {
        ...
        {
        return state.DoS(0, error("ConnectBlock(EVOMASTERNODES): couldn't find evo masternode payments"),
                                REJECT_INVALID, "bad-cb-payee");
        }

        LogPrintf("IsBlockPayeeValid -> masternode rules not enforced!\n");
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensus, CBlockIndex * const pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Check timestamp
    if (block.GetBlockTime() > nAdjustedTime + 3 * 60 * 60)
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    const CChainParams& chainparams = Params();
    int nLockTimeFlags = 0;
    if (nHeight >= chainparams.GetConsensus().CSV)
    {
    ...
    // failed).
    if (GetBlockWeight(block) > MAX_BLOCK_BASE_SIZE) {
...
bool LoadExternalBlockFile(const CChainParams& chainparams, FILE* fileIn, CDiskBlockPos *dbp)
{
    ...
    try {
        ...
        CBufferedFile blkdat(fileIn, 2*MAX_BLOCK_BASE_SIZE, MAX_BLOCK_BASE_SIZE+8, SER_DISK, CLIENT_VERSION);
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // read size
                blkdat >> nSize;
                if (nSize < 80 || nSize > MAX_BLOCK_BASE_SIZE)
                    continue;
